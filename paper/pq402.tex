...
\subsection{Where this sits in Stellar's own quantum plan}

On 4 August 2026, days before this was written, the Stellar Development
Foundation published its Quantum Preparedness
Plan.\footnote{\texttt{stellar.org/blog/foundation-news/introducing-the-quantum-preparedness-plan}}
It is a signature-migration plan first: ML-DSA-44 and ML-DSA-65 as Soroban host
functions in 2026, quantum-safe signers on classic accounts in 2027, Ed25519
retired when the threat demands it. None of that is what this work does; account
authentication is a different layer, and the disanalogy should be stated rather
than blurred.

The plan is candid about what it does not cover, and that paragraph is this
work's exact address:

\begin{quote}
``The QPP does not yet address pairing-based zero-knowledge protocols built on
Stellar\ldots there is no drop-in post-quantum replacement for pairing-based
SNARKs with comparable performance. STARKs and lattice-based proof systems are
candidates, each with significant trade-offs. As part of QPP we will convene ZK
protocol teams\ldots to develop a shared research agenda.''
\end{quote}

Two of the plan's own words carry the point. The zero-knowledge layer is the
\emph{unfinished} half of Stellar's quantum migration, and STARKs are one of the
two \emph{named} candidates for it. This system is a working instance of that
candidate --- a Circle STARK verified inside a Soroban contract --- and the
``significant trade-offs'' the plan gestures at are the ones the Measurements
section puts numbers on: a 196\,KB proof filling 86\% of the transaction
envelope, roughly 96 conjectured bits of soundness, and about twice the
instruction cost of a pairing. That is not an endorsement of this work by the
foundation, which the plan does not give and this paper does not claim. It is a
data point of exactly the kind the plan says is missing, arriving as the
foundation asks for them.
...
Soroban per-transaction instruction ceiling & 400{,}000{,}000 instr. \\
transaction envelope, unlinkable act & 114{,}084 B \\
transaction envelope, identifying spend & 77{,}508 B \\
Soroban per-transaction size ceiling & 132{,}096 B \\
...
Two things fall out of them. The first is which ceiling actually binds, and it
is not the one we expected. Instructions sit at 61\% of budget, which sounds
like room; the envelope sits at \textbf{86\%} of the 132{,}096-byte transaction
size cap, which is not room at all. The proof has to travel to the chain before
it can be checked there, and on Soroban the trip is scarcer than the check.
Every reference to a ``400M instruction cap'' as the limiting resource,
including two we made earlier in this work, was measuring the wrong wall.

The second is worth pausing on. The identifying gate asks 40 queries at blowup 2
and costs \emph{more} --- 265.8M against 243.2M --- while being the weaker of
the two by a wide margin. Blowup buys soundness at a discount that queries do
not: raising the rate lengthens the committed codeword, which the prover pays
for, while the verifier's cost tracks the number of openings it has to check. On
a chain that meters the verifier and not the prover, few queries over a high
blowup wins on both cost and soundness.

It does not win on everything, and the obvious next move is a trap. Pushing the
blowup higher again would buy more soundness for a few percent more
instructions, and would quietly destroy the hiding property. Counting the FRI
phase as a linear-algebra problem --- what the verifier observes against what
the blinding can cover --- gives observations $Q \cdot \ell$ with
$\ell = \log_2(2N \cdot 2^\beta)$, against $3N$ dimensions of blinding: the
trace blinder, collapsed to $N$ by the $\alpha$-reduction, plus the $2N$ of the
randomisation polynomial. So soundness improves as $Q\beta$ while the
zero-knowledge margin degrades as $Q(\log_2 2N + \beta)$, and zero-knowledge
loses the race because it pays the committed height on top of the blowup. A
negative margin is conclusive in the bad direction: fewer blinding dimensions
than revealed scalars means some linear functional of the witness survives into
the transcript, and no simulator of this shape exists.

This is why the contracts \emph{pin} the configuration rather than enforce a
floor. A floor was the first fix, and it was the wrong one: a caller passing a
larger blowup would have been accepted as stronger while silently destroying
hiding. There is no scalar to be above --- there is one configuration checked on
all four axes at once, and anything else is a different configuration that has
not been. The lever that helps both directions is trace height, which adds three
blinding dimensions per row against $Q$ observations per doubling.

And that is where the four constraints close on each other, which is the
result this section exists to report. The deployed configuration --- 128 rows,
12 queries, blowup $2^7$ --- sits at 92 conjectured bits of soundness, a FRI
zero-knowledge margin of $+204$, 61\% of the instruction budget, and 86\% of the
envelope. Each available move breaks one of the other three. Raising the blowup
buys soundness and drives the margin toward zero, which is conclusive against
hiding. Raising the queries buys soundness and adds a Merkle path per query to
an envelope already at 86\%. Raising the trace height is the only move that
helps soundness and hiding together, and it is precisely the move that doubles
the proof and overruns the size cap first. The configuration is not a
preference. It is the last point that satisfies all four at once, and the reason
it looks arbitrary is that three of the four walls are invisible until measured.
...
That relocation is also what explains the gap in the numbers. Soroban's
per-transaction limits, not the cryptography, are what hold the security
parameters down --- and the binding one is the 132{,}096-byte envelope rather
than the instruction budget, as the measurements below show. A software verifier
with 20--30\,ms to spend has no such ceiling and no need to ship the proof
anywhere. Closing that gap means smaller proofs rather than a faster verifier,
which is exactly what CAPSS and SmallWood are for, and is where we would start.
