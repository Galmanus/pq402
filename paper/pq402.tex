...
We know of no prior x402 payment gated by an unlinkable credential, on Stellar
or elsewhere, and no prior STARK verified directly by a Soroban contract:
everything verifying zero-knowledge proofs on Soroban today is pairing-based,
and RISC~Zero reaches Stellar only after a Groth16 wrapper that reintroduces
pairings and a trusted setup at the last step. That is a statement about what
public search surfaced on 2 August 2026, not a proof of absence. If it is
wrong, the correction belongs in the repository and will be put there.
